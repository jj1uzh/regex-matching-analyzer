...
class DetNoAssertTreeTransducer[Q, A](
  ...
  def toTotal(): DetNoAssertTreeTransducer[(Q, Set[Q]), A] = {
    ...
    def makeDFA(td: DetNoAssertTreeTransducer[Q, A]): DFA[Set[Q], A] = {
      ...
      val dfa_finalstates =
        dfa_states.filter(q => q.subsetOf(statesCanEOFtrans)) // only sets contains EOF trans
    ...
    for ((q, s) <- newstates; a <- this.sigma) {
      Analysis.checkInterrupted("preparing for calculate growth rate")
      if (
        this.deltaDet.get((q, Some(a))) != None && dfa.deltaDet.get((s, a)) != None && (usefulstate
          .contains(dfa.deltaDet((s, a))))
      ) {
        // delta(q,a) is acceptable in DFA
        newdeltaDet += (((q, s), Some(a)) -> (this.deltaDet((q, Some(a))) >>= { case q =>
          TreeMonad.unit((q, dfa.deltaDet((s, a))))
        }))
      } else {
