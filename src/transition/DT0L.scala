...
class DT0L[A, Q](
  ...
  def calcGrowthRate(initials: Set[Q]): (Option[Int], Witness[A], Option[Q]) = {
    ...
    def calcGrowthRate(): (Option[Int], Seq[Pump]) = {
      ...
      val scEdges = graph.edges
        .map { case (q1, q2) =>
          (scMap(q1), scMap(q2))
        }
        .filter { case (sc1, sc2) => sc1 != sc2 }
        .distinct
      ...
      def checkEDA(): Option[Pump] = {
        ...
        scGraph.nodes.filterNot(isAtom).to(LazyList).map(checkEDA).find(_.isDefined).map(_.get)
      }
      ...
      def calcDegree(): (Int, Seq[Pump]) = {
        def checkIDA(start: Set[Q], end: Set[Q]): Option[Pump] = {
          def constructG3(): LabeledGraph[(Q, Q, Q), A] = {
            ...
            val labeledAdjPassage =
              for (sc1 <- passage; sc2 <- passage) yield scPairLabeledAdj((sc1, sc2))
          ...
          val g3 = constructG3()
          val e3WithBack = g3.labeledEdges.map { case (v1, _, v2) =>
            (v1, v2)
          } ++ (for (
            p <- start;
            q <- end
          ...
          g3WithBack
            ...
            .map { sc =>
              sc.collect {
                ...
                sc.exists { case (q1, q2, q3) => q1 == p && q2 == p && q3 == q }
              }.map { case (p, q) =>
                (p, g3.getPath((p, p, q), (p, q, q)).get, q)
              }
            }
            .find(_.isDefined)
            .map(_.get)
    ...
    def calcGrowthRate(): (Option[Int], Seq[Pump]) = {
      ...
      def calcDegree(): (Int, Seq[Pump]) = {
        def checkIDA(start: Set[Q], end: Set[Q]): Option[Pump] = {
          ...
          val g3 = constructG3()
          val e3WithBack = g3.labeledEdges.map { case (v1, a, v2) =>
            (v1, Some(a), v2)
          } ++ (for (
            (r1, p1) <- start;
            (r2, p2) <- end;
          ...
          g3WithBack
            ...
            .map { sc =>
              ...
              }.find { case (r11, r12, p1) =>
                sc.exists { case ((r21, r22, r23), p2) =>
                  r21 == r11 && r22 == r12 && r23 == r12 && p1 == p2
                }
              }.map { case (r11, r12, p1) =>
                ((r11, p1), g3.getPath(((r11, r11, r12), p1), ((r11, r12, r12), p1)).get, (r12, p1))
              }
            }
            .find(_.isDefined)
            .map(_.get)
