...
sealed trait Tree[+A] {
  def find(p: A => Boolean): Option[A] = this match {
    ...
    case Or(l, r)        => l.find(p) orElse r.find(p)
    case Lft(l)          => l.find(p)
    case _               => None
  }
}
...
object Tree {
  ...
  implicit object SetTreeMonad extends Monad[SetTree] {
    ...
    def success[A] = Set(Success)
    def fail[A] = Set(Fail)
    def concat[A](m1: SetTree[A], m2: SetTree[A]) =
      m1.flatMap(tree1 => m2.flatMap(tree2 => Set(Or(tree1, tree2))))
  ...
  def pruneNoAssert[Q](t: Tree[Q], qs: Set[Q] = Set[Q]()): Tree[Q] = {
    t match {
      case Or(l, r) =>
        if (TreeMonad.eval(l)(qs)) Lft(pruneNoAssert(l, qs)) else Or(l, pruneNoAssert(r, qs))
      case _ => t
    }
  }
}
