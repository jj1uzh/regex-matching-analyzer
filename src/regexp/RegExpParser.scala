        ...
        def quantifiedFactor: Parser[RegExp[Char]] = {
          def factor: Parser[RegExp[Char]] = {
            def elem: Parser[ElemExp[Char]] = {
              ...
              def specialMeta: Parser[Char] = "\\" ~> spacialMetas ^^ { c =>
                convertSpecialMeta(c.head)
              }
            ...
            def charClass: Parser[CharClassExp] = {
              def charClassFactors: Parser[Seq[CharClassElem]] = {
                def charClassElem: Parser[Seq[Char]] = {
                  ...
                  def charClassSpecialMeta: Parser[Char] = "\\" ~> spacialMetasInCharClass ^^ { c =>
                    convertSpecialMeta(c.head)
                  }
                  def oct: Parser[Seq[Char]] = "\\" ~> """\d+""".r ^^ { d =>
                    val (octalPart, elemsPart) = d.take(3).span(_ < '8')
                    s"""${Integer.parseInt(s"0${octalPart}", 8).toChar}${(elemsPart + d.drop(3))}"""
                  }
              ...
              "[" ~> opt("^") ~ charClassFactors <~ "]" ^^ { case neg ~ fs =>
                CharClassExp(fs, neg.isEmpty)
              }
            }
            ...
            def ifCond: Parser[IfExp[Char]] =
              ("(?(" ~> exp <~ ")") ~ term ~ opt("|" ~> exp) <~ ")" ^^ { case cond ~ rt ~ rf =>
                IfExp(cond, rt, rf.getOrElse(EpsExp()))
              }
            ...
            def unicode: Parser[Char] = "\\u" ~> "[0-9a-fA-F]{0,4}".r ^^ { code =>
              if (code.length == 4) {
                ...
                throw RegExpParser.ParseException(s"invalid unicode representation: \\u${code}")
            }
          ...
          def quantifier: Parser[Either[(Option[Int], Option[Int]), String]] = {
            ...
            def repeat: Parser[Left[(Option[Int], Option[Int]), Nothing]] = {
              def minmax: Parser[(Option[Int], Option[Int])] =
                ("{" ~> opt(posNum) <~ ",") ~ opt(posNum) <~ "}" ^^ { case min ~ max =>
                  (min, max)
                }
              def exact: Parser[(Option[Int], Option[Int])] = "{" ~> posNum <~ "}" ^^ { case num =>
                (Some(num), Some(num))
              }
