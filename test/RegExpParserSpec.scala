...
class RegExpParserSpec extends AnyFlatSpec with Matchers {
  ...
  it should "parse octal representation" in {
    ...
    RegExpParser("\\81") should be(
      ConcatExp(ConcatExp(ElemExp('\u0000'), ElemExp('8')), ElemExp('1'))
    )
  }
  ...
  it should "parse character class" in {
    ...
    RegExpParser("""[ab-def-h]""") should be(
      CharClassExp(
        Seq(
          SingleCharExp('a'),
          RangeExp('b', 'd'),
  ...
  it should "parse complex expression" in {
    ...
    RegExpParser("""(?(a)bc|d|e)""") should be(
      IfExp(
        ElemExp('a'),
