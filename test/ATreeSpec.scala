...
class ATreeSpec extends AnyFlatSpec with Matchers {
  ">>=l" should "calculate left bind" in {
    ...
    (t `>>=l` (c =>
      AOr(ALeaf(c.toString), ALeaf(c.toUpper.toString)): ATree[String, String]
    )) should be(
      AOr(ALeaf(1), AAssert(AOr(AFail(), AOr(ALeaf("a"), ALeaf("A"))), ALeaf(2))): ATree[
        String,
        Int
      ]
    )
