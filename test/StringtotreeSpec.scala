...
class stringtotreeTest extends AnyFlatSpec {
  "toDeterministic " should "success" in {
    ...
    assert(
      td2.deltaDet(((1, 1), Some(Left('a')))) == Leaf((1, 1)) && td2.deltaDet(
        ((1, 2), Some(Left('a')))
      ) == Leaf((2, 1)) && td2.deltaDet(((1, 3), (Some(Left('a'))))) == Leaf((3, 1))
    )
  ...
  "Bprune" should "success" in {
    ...
    val td1 =
      new NonDetNoAssertTreeTransducer[Int, Option[Char]](states1, sigma1, initialState, transition)

    val td2 = td1.Bprune()
    assert(
      td2.delta(((true, None), Some(None))) == Set(Leaf((true, Some(1))), Leaf((false, Some(1))))
    )
    assert(
      td2.delta(((true, Some(1)), Some(Some('a')))) == Set(
        Lft(Leaf((true, Some(1)))),
        Or(Leaf((false, Some(1))), Leaf((true, Some(2)))),
        Leaf((true, Some(3)))
      )
    )
